...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        num_categories_available = len(categories)

        for _ in range(10):  # Generate 10 fake products
            user = user
            num_categories = min(random.randint(1, 4), num_categories_available)
            ...
            status = random.choice([choice[0] for choice in ProductStatusType.choices])  # Corrected status choice
